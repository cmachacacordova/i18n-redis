...
struct adl_serializer<i18n::Translation> {
  static void to_json(nlohmann::json &j, const i18n::Translation &t) {
    j["id"] = t.id;
    j["value"] = t.value;
  ...
  static void from_json(const nlohmann::json &j, i18n::Translation &t) {
    j.at("id").get_to(t.id);
...
bool i18n::RedisTranslationProvider::load(const std::string &cwd, const std::vector<std::string> &locales) const {
  for (const auto &locale : locales) {
    const std::filesystem::path localePath =
        (std::filesystem::path(cwd) / "locales" / locale).lexically_normal();
    ...
    for (const auto &entry : std::filesystem::directory_iterator(localePath)) {
      ...
      i18n::json j;
      file >> j;
      const auto translations = j.get<std::vector<i18n::Translation>>();

      for (const auto &translation : translations) {
        if (translation.id.empty()) {
          throw std::runtime_error(
              fmt::format("Translation ID is empty in file: {}", entry.path().string()));
        }
        if (translation.value.empty()) {
          throw std::runtime_error(fmt::format(
              "Translation value is empty for ID '{}' in file: {}", translation.id, entry.path().string()));
        }
        if (translation.category.empty()) {
          throw std::runtime_error(fmt::format(
              "Translation category is empty for ID '{}' in file: {}", translation.id, entry.path().string()));
        }
        if (translation.creationDate.empty()) {
          throw std::runtime_error(fmt::format(
              "Translation creationDate is empty for ID '{}' in file: {}", translation.id, entry.path().string()));
        }
        if (translation.modificationDate.empty()) {
          throw std::runtime_error(fmt::format(
              "Translation modificationDate is empty for ID '{}' in file: {}", translation.id, entry.path().string()));
        }
        if (translation.modificationVersion < 0) {
          throw std::runtime_error(fmt::format(
              "Translation modificationVersion is negative for ID '{}' in file: {}", translation.id, entry.path().string()));
        }
        if (translation.id.find(':') != std::string::npos) {
          throw std::runtime_error(fmt::format(
              "Translation ID '{}' contains a colon, which is not allowed in Redis keys.", translation.id));
        }
...
std::string i18n::RedisTranslationProvider::get(const std::string &key, const std::string &locale) const {
  if (const auto value = this->connection.value<i18n::Translation>(fmt::format(I18N_FORMAT_KEY, locale, key))) {
    return value->value;
  }
  return key;
}
